...
def main():
    parser = argparse.ArgumentParser(description='Create Solr queries config')
    ...
    queries_core_path = os.path.join(args.out_config_dir, f'grebi_queries_{args.subgraph_name}')
    os.system('cp -r ' + shlex.quote(os.path.join(args.in_template_config_dir, "grebi_queries")) + ' ' + shlex.quote(queries_core_path))
    ...
    Path(f'{queries_core_path}/core.properties').write_text(f"name=grebi_queries_{args.subgraph_name}\n")
